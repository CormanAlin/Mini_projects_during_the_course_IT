...
class TestUser(unittest.TestCase):
  def test_add_user(self):
    ...
    message = user.save()
    # expectation
    was_user_crated = True
    try:
      f = open("user_user1") # we try to read the file
      f.close() # close the file if we managed to open is
    except:
      was_user_crated = False # if we fail, we set to False

    self.assertTrue(was_user_crated)
  ...
  def test_user_already_exists(self):
    ...
    f.close() # close the file after we crated
    # execution
    user = User("user1")
    message = user.save()
    # cleanup
